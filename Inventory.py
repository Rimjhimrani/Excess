...
class InventoryManagementSystem:
    ...
    def display_enhanced_analysis_charts(self, analysis_results):
        """Display visual summaries using Total Actual (Bar) vs Total Ideal (Line) with exact requested colors and hovers"""
        ...
        if value_col and 'PART NO' in df.columns:
            ...
            res = chart_data.apply(calc_part_metrics, axis=1, result_type='expand')
            ...
            chart_data['Status_Key'] = chart_data['Status'] if 'Status' in chart_data.columns else 'Within Norms'
            
            fig1 = go.Figure()
            for status, color in color_map.items():
                subset = chart_data[chart_data['Status_Key'] == status]
                if not subset.empty:
                    fig1.add_trace(go.Bar(
                        x=subset['Part_Label'], y=subset['Actual_V'], name=status, marker_color=color,
                        customdata=subset.apply(lambda r: f"Description: {r['PART DESCRIPTION']}<br>Part No: {r['PART NO']}<br><b>Status: {r['Status_Key']}</b><br>Actual: ₹{r[value_col]:,.0f}<br>Ideal: ₹{(r['Ideal_V']*divisor):,.0f}<br>Dev: {r['Dev_Pct']:.1f}%", axis=1),
                        hovertemplate='%{customdata}<extra></extra>'
                    ))
            
            fig1.add_trace(go.Scatter(x=chart_data['Part_Label'], y=chart_data['Ideal_V'], name='Ideal Target', line=dict(color='black', width=1.5), marker=dict(size=6, color='black')))
            fig1.update_layout(title=f"Top Parts: Actual Value vs Ideal Target", xaxis_tickangle=-45, yaxis_ticksuffix=suffix, height=550)
        ...
        if vendor_col and value_col:
            st.markdown(f"### 🏢 Top {top_n} Vendors: Actual vs Target")
            v_group = df.groupby(vendor_col).agg({value_col: 'sum', 'AVG CONSUMPTION/DAY': 'sum', 'UNIT PRICE': 'mean'}).reset_index()
            
            def calc_v_metrics(row):
                ac, up = float(row.get('AVG CONSUMPTION/DAY', 0) or 0), float(row.get('UNIT PRICE', 0) or 0)
                ideal_v = (ac * ideal_days * up) / divisor
                actual_v = row[value_col] / divisor
                return actual_v, ideal_v

            v_res = v_group.apply(calc_v_metrics, axis=1, result_type='expand')
            v_group['Actual_V'], v_group['Ideal_V'] = v_res[0], v_res[1]
            v_group = v_group.sort_values('Actual_V', ascending=False).head(top_n)

            fig2 = go.Figure()
            fig2.add_trace(go.Bar(x=v_group[vendor_col], y=v_group['Actual_V'], name='Actual Value', marker_color='#FF9800', hovertemplate='Vendor: %{x}<br>Actual: ₹%{y:.2f}'+suffix+'<extra></extra>'))
            fig2.add_trace(go.Scatter(x=v_group[vendor_col], y=v_group['Ideal_V'], name='Ideal Target', line=dict(color='black', width=1.5), marker=dict(size=6, color='black')))
            fig2.update_layout(title="Top Vendors: Actual vs Target", xaxis_tickangle=-45, yaxis_ticksuffix=suffix, height=500)
            st.plotly_chart(fig2, use_container_width=True)

        # ==========================================
        # 3. Top Parts by Status
        # ==========================================
        st.markdown(f"## 🧩 Top {top_n} Parts by Status")
        for status, color in [("Excess Inventory", "#2196F3"), ("Short Inventory", "#F44336")]:
            ...
            st.subheader(f"{'🔵' if 'Excess' in status else '🔴'} {status}")
            
            sm = status_df.apply(calc_part_metrics, axis=1, result_type='expand')
            status_df['Actual_V'], status_df['Ideal_V'] = sm[0], sm[1]
            status_df['Label'] = status_df.apply(lambda r: f"{r['PART DESCRIPTION'][:20]}... ({r['PART NO']})", axis=1)

            fig3 = go.Figure()
            fig3.add_trace(go.Bar(
                x=status_df['Label'], y=status_df['Actual_V'], name='Actual Value', marker_color=color,
                customdata=status_df.apply(lambda r: f"Part: {r['PART NO']}<br>Actual: ₹{r[value_col]:,.0f}<br>Ideal Target: ₹{(r['Ideal_V']*divisor):,.0f}<br>Deviation: ₹{abs(r['Stock Deviation Value']):,.0f}", axis=1),
                hovertemplate='%{customdata}<extra></extra>'
            ))
            fig3.add_trace(go.Scatter(x=status_df['Label'], y=status_df['Ideal_V'], name='Ideal Target', line=dict(color='black', width=1.5), marker=dict(size=6, color='black')))
            fig3.update_layout(title=f"Top {status} Parts: Actual vs Target", xaxis_tickangle=-45, yaxis_ticksuffix=suffix, height=550)
            st.plotly_chart(fig3, use_container_width=True, key=f"ps_{status}")
        ...
        for status, color in [("Excess Inventory", "#2196F3"), ("Short Inventory", "#F44336")]:
            ...
            st.subheader(f"{'🔵' if 'Excess' in status else '🔴'} Vendors - {status}")
            ...
            vsm = v_stat.apply(calc_v_metrics, axis=1, result_type='expand')
            v_stat['Actual_V'], v_stat['Ideal_V'] = vsm[0], vsm[1]

            fig4 = go.Figure()
            fig4.add_trace(go.Bar(x=v_stat[vendor_col], y=v_stat['Actual_V'], name='Actual Value', marker_color=color, hovertemplate='Vendor: %{x}<br>Actual: ₹%{y:.2f}'+suffix+'<extra></extra>'))
            fig4.add_trace(go.Scatter(x=v_stat[vendor_col], y=v_stat['Ideal_V'], name='Ideal Target', line=dict(color='black', width=1.5), marker=dict(size=6, color='black')))
            fig4.update_layout(title=f"Top {status} Vendors: Actual vs Target", xaxis_tickangle=-45, yaxis_ticksuffix=suffix, height=550)
            st.plotly_chart(fig4, use_container_width=True, key=f"vs_{status}")
